...
def capture_image():
    ...
    frame, emotion = video_camera.capture_frame()  
    ...
    if frame is not None:
        ...
        # Fetch user age
        cursor.execute("SELECT age FROM user WHERE username = %s", (session['username'],))
        user_data = cursor.fetchone()

        if not user_data:
            cursor.close()
            return jsonify({"error": "User age not found"}), 500
        ...
        # Retrieve songs based on both age category and emotion
        cursor.execute("SELECT song FROM song WHERE emotion = %s AND username = %s", (emotion, age_category))
        songs = [row["song"] for row in cursor.fetchall()]
...
def regsiter():
    ...
    if request.method=='POST':
        ...
        if cnt==0:
            ...
            if maxid is None:
                maxid=1
            sql = "INSERT INTO user(id, name, mobile, email, address, username, password, age,date_join) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)"
            val = (maxid, name, mobile, email, address, username, password, age,date_join)
...
def add_song():
    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            random_filename = ''.join(random.choices(string.ascii_letters + string.digits, k=10)) + "." + ext
            ...
            val = (maxid, emotion, random_filename, age, date_join)
            mycursor.execute(sql, val)
            mydb.commit()
            msg = "success"
